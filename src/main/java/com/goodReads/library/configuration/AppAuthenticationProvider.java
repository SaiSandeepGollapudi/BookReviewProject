...
public class AppAuthenticationProvider implements AuthenticationProvider {// after spring security any request that comes spring will first check if there's any implementation
    ...
    @Autowired
    UserDetailsService userDetailsService;// we use interface UserDetailsService because we can use another implementation of MongoDB also along with MySQl
    @Autowired
...
// * 2. accept/ retrieves username and password when the user inputs them by AuthenticationProvider class via Authentication object
@Override //Authentication object -> it will hold the details of the username and password which has been provided by the user.
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {// AuthenticationProvider provides with the authentication object and
    // Authentication which is a bean/ object provided by spring security it will authenticate the user
    ...
    // use below line to set the username from name.
        ...
        if(Objects.nonNull(userDetails)){  // check if the user details is not null
            if(encoder.matches(authentication.getCredentials().toString(),userDetails.getPassword())){// check if the password matches
                ...
                // Username, password and authorities are needed to create a sessionId/toke for us.
            }
        }
    ...
    @Override
    public boolean supports(Class<?> authentication) {// check if the authentication object like passwords, passkeys, token is supported
